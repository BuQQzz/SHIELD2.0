...
function displayResults(query, searchData, prompt) {
  ...
  console.log(`${colors.yellow}${colors.bright}SEARCH RESULTS (${searchData.results.length})${colors.reset}\n`);
  searchData.results.forEach((result, index) => {
  ...
  // Display the prompt
  console.log('='.repeat(80));
  console.log(`${colors.green}${colors.bright}PROMPT SENT TO LLM${colors.reset}`);
  console.log('='.repeat(80));
  console.log(prompt);
  console.log('='.repeat(80) + '\n');
  ...
  console.log(`Contains date context: ${prompt.includes('Current Date:') ? 'YES ✓' : 'NO ✗'}`);
  console.log(`Contains critical instructions: ${prompt.includes('CRITICAL INSTRUCTIONS') ? 'YES ✓' : 'NO ✗'}`);
  console.log(`Contains absolute rules: ${prompt.includes('ABSOLUTE RULES') ? 'YES ✓' : 'NO ✗'}`);
  console.log('');
}
...
async function runTest() {
  ...
  console.log(`${colors.cyan}${colors.bright}║                    WEB SEARCH FLOW TEST SCRIPT                             ║${colors.reset}`);
  ...
  log(colors.yellow, 'NOTE', 'The prompts above show exactly what the LLM receives.');
  log(colors.yellow, 'NOTE', 'If the LLM ignores these instructions, the issue is with the model itself.');
  console.log('');
}
