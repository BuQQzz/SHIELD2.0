...
export function createMessageHandler({
  ...
  return async (content: string, useWebSearch?: boolean) => {
    console.log("[MessageHandler] Called with content:", content, "useWebSearch:", useWebSearch);
    ...
    const isVagueFollowUp = vagueFollowUpPatterns.some((pattern: RegExp) => pattern.test(content.trim()));
    
    if (isVagueFollowUp) {
      console.log("[MessageHandler] Skipping web search for vague follow-up query:", content);
    }
    
    if (useWebSearch && performWebSearch && !isVagueFollowUp) {
      console.log("[MessageHandler] Web search requested for query:", content);
      console.log("[MessageHandler] performWebSearch function exists:", !!performWebSearch);
      ...
      try {
        ...
        if (searchData && searchData.contents.length > 0) {
          console.log(`[MessageHandler] Building context from ${searchData.contents.length} full pages and ${searchData.results.length} search results`);
          ...
          if (searchData.results.length > searchData.contents.length) {
            ...
            searchData.results.slice(searchData.contents.length).forEach((result, index) => {
          ...
          webSearchContext += "You are answering based on CURRENT LIVE WEB SEARCH RESULTS shown above. These results were just fetched from the internet.\n\n";
          webSearchContext += "ABSOLUTE RULES:\n";
          webSearchContext += "1. ONLY use information that appears in the search results above\n";
          webSearchContext += "2. If search results say something different from your training data, YOU MUST USE THE SEARCH RESULTS\n";
          webSearchContext += "3. DO NOT make up or assume any information not in the results\n";
          webSearchContext += "4. DO NOT list sources in your response - they will be shown separately\n";
          webSearchContext += "5. If information is unclear or contradictory in results, SAY SO\n";
          webSearchContext += "6. Today's date is: " + new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) + "\n\n";
          webSearchContext += "Your response MUST be based on the search results above, NOT your training data.\n\n";
          
          // Debug: Log what we're sending to the LLM
          console.log("[MessageHandler] Full content search context being sent to LLM:");
    ...
    try {
      ...
      console.log("[MessageHandler] Final prompt being sent (first 500 chars):");
      ...
      if (webSearchContext) {
        ...
        if (reasoningMatch && reasoningMatch[1]) {
          ...
          if (!settings.webSearch.showReasoning) {
            processedContent = finalContent.replace(/<reasoning>[\s\S]*?<\/reasoning>\s*/, '').trim();
          } else {
