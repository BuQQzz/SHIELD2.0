...
interface MessageProps {
  ...
  onRegenerate?: () => void;
  /**
   * "part": one step of an assistant reply, drawn inside AssistantTurn,
   * which supplies the avatar, name, actions and stats once for the reply.
   */
...
export const ChatMessage = memo(function ChatMessage({
  ...
  const { content, pendingTool, pendingTarget } = useMemo(() => {
    if (role !== "assistant")
      return {
        content: rawContent,
        pendingTool: null,
    ...
    if (isStreaming) {
      ...
      return {
        ...
        pendingTool: preview.pendingTool,
        pendingTarget: preview.pendingTarget,
      };
    }
    return {
  ...
  // The text of the message, and what follows it (Continue, sources).
      ...
      {isEditing ? (
        ...
        <div className="space-y-2">
          ...
          <div className="text-sm text-muted-foreground prose prose-sm max-w-none dark:prose-invert">
            ...
            {pendingTool !== null && (
              <div className="not-prose mt-1 flex items-center gap-2 text-[13px] text-muted-foreground">
                <Loader2 className="h-3.5 w-3.5 animate-spin" />
                <span>
                  {pendingTool
                    ? `${stepLabel(pendingTool, "filesystem", pendingTarget, "running")}…`
