    ...
    results: [
      {
        ...
        snippet: "The Outer Worlds 2 is scheduled for release on PlayStation 5, Xbox Series X|S, and PC in 2025. The game was announced at E3 2021 and is currently in development by Obsidian Entertainment.",
        url: "https://www.gamesradar.com/outer-worlds-2-release-date/"
      },
      {
        title: "Outer Worlds 2: Everything We Know - IGN",
        snippet: "As of November 2025, The Outer Worlds 2 has not yet been released. The game is still in development and no specific release date has been announced beyond '2025'.",
        url: "https://www.ign.com/articles/outer-worlds-2-release-date"
      },
      {
        title: "Is The Outer Worlds 2 Out Yet? - GameSpot",
        snippet: "The Outer Worlds 2 is not out yet. Obsidian Entertainment has confirmed the game is still in active development for a 2025 release window on PS5, Xbox Series X|S, and PC.",
        url: "https://www.gamespot.com/articles/outer-worlds-2-release/"
      },
      {
        title: "The Outer Worlds 2 Release Date, News, and Trailers",
        snippet: "The Outer Worlds 2 was originally announced for a 2025 release. As of now, the game has not launched and Obsidian has not provided an exact release date.",
    ...
    results: [
      {
        ...
        snippet: "Here are some polite ways to ask someone to verify information: 'Would you mind double-checking?', 'Could you please review this once more?', 'I'd appreciate if you could take another look.'",
        url: "https://www.grammarly.com/blog/polite-requests/"
      },
      {
        title: "Professional Email Phrases for Follow-ups",
        snippet: "When following up on a request, use phrases like: 'Could you please check again?', 'I wanted to follow up on my previous request', 'Would it be possible to review this again?'",
...
function buildPrompt(query, searchData) {
  ...
  if (searchData.results.length > 0) {
    ...
    webSearchContext += "You are answering based on CURRENT LIVE WEB SEARCH SNIPPETS shown above. These were just fetched from the internet.\n\n";
    webSearchContext += "ABSOLUTE RULES:\n";
    webSearchContext += "1. ONLY use information from the snippets above\n";
    webSearchContext += "2. Search results OVERRIDE your training data - use them instead\n";
    webSearchContext += "3. DO NOT invent information not in the snippets\n";
    webSearchContext += "4. DO NOT list sources - they will be shown separately\n";
    webSearchContext += "5. If snippets are unclear or limited, say \"Based on the search results...\"\n";
    webSearchContext += "6. Today's date is: " + new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }) + "\n\n";
...
function displayResults(query, searchData, prompt) {
  console.log('\n' + '='.repeat(100));
  console.log(`${colors.magenta}${colors.bright}QUERY: ${query}${colors.reset}`);
  ...
  console.log(`${colors.yellow}${colors.bright}SEARCH RESULTS (${searchData.results.length} found)${colors.reset}\n`);
  searchData.results.forEach((result, index) => {
    console.log(`${colors.cyan}${colors.bright}${index + 1}. ${result.title}${colors.reset}`);
  ...
  // Display the prompt
  console.log('='.repeat(100));
  console.log(`${colors.green}${colors.bright}FULL PROMPT SENT TO LLM (${prompt.length} chars)${colors.reset}`);
  console.log('='.repeat(100));
  console.log(prompt);
  console.log('='.repeat(100) + '\n');
  ...
  console.log(`├─ Contains current date: ${prompt.includes('Current Date:') ? `${colors.green}YES ✓${colors.reset}` : `${colors.red}NO ✗${colors.reset}`}`);
  console.log(`├─ Contains critical instructions: ${prompt.includes('CRITICAL INSTRUCTIONS') ? `${colors.green}YES ✓${colors.reset}` : `${colors.red}NO ✗${colors.reset}`}`);
  console.log(`├─ Contains absolute rules: ${prompt.includes('ABSOLUTE RULES') ? `${colors.green}YES ✓${colors.reset}` : `${colors.red}NO ✗${colors.reset}`}`);
  console.log(`└─ Has user question marker: ${prompt.includes('===== USER\'S QUESTION =====') ? `${colors.green}YES ✓${colors.reset}` : `${colors.red}NO ✗${colors.reset}`}`);
  console.log('');
}
...
function runTest() {
  ...
  console.log(`${colors.cyan}${colors.bright}║                         WEB SEARCH PROMPT ANALYSIS TEST SCRIPT                                   ║${colors.reset}`);
  ...
  console.log(`${colors.green}${colors.bright}EXPECTED LLM BEHAVIOR:${colors.reset}`);
  console.log(`The LLM should answer: "No, The Outer Worlds 2 has not been released yet on PS5."`);
  console.log(`It should use information from the search results that clearly state:`);
  ...
  console.log(`${colors.red}${colors.bright}HALLUCINATION EXAMPLE (what we saw before):${colors.reset}`);
  console.log(`"Yes, The Outer Worlds 2 is now available for PS5. It launched globally on October 29, 2023."`);
  console.log(`This is WRONG - it ignores the search results and uses outdated training data.`);
  console.log('\n\n');
  ...
  console.log(`${colors.red}${colors.bright}PROBLEM IDENTIFIED:${colors.reset}`);
  console.log(`When user asks "please check again", DuckDuckGo returns results about "how to politely ask someone to check"`);
  console.log(`Instead of re-checking the previous topic (Outer Worlds 2), it searches for the literal phrase.`);
  console.log('');
  ...
  console.log(`  A) Detect follow-up questions and skip web search (use conversation context only)`);
  console.log(`  B) Include previous query context when searching: "outer worlds 2 ps5 release check"`);
  console.log(`  C) Disable web search for very short/vague queries like "please check again"`);
  ...
  console.log('='.repeat(100));
  console.log('');
  console.log(`${colors.yellow}Issue #1: LLM Ignoring Search Results${colors.reset}`);
  console.log(`  - Prompt clearly states "ABSOLUTE RULES: ONLY use information from snippets"`);
  console.log(`  - Search results say "not yet released"`);
  console.log(`  - LLM says "released on October 29, 2023"`);
  console.log(`  ${colors.red}→ Model is favoring training data over explicit instructions${colors.reset}`);
  console.log('');
  console.log(`${colors.yellow}Issue #2: Follow-up Context Loss${colors.reset}`);
  console.log(`  - User asks "please check again" expecting to re-check Outer Worlds 2`);
  console.log(`  - System searches for "please check again" literally`);
  console.log(`  - Returns unrelated results about polite requests`);
  console.log(`  ${colors.red}→ No conversation context in search queries${colors.reset}`);
  console.log('');
  console.log(`${colors.green}${colors.bright}RECOMMENDED FIXES:${colors.reset}`);
  console.log(`  1. ${colors.cyan}Lower temperature${colors.reset} for web search queries (0.3 instead of 0.7)`);
  console.log(`  2. ${colors.cyan}Add system message${colors.reset} specifically for web search mode`);
  console.log(`  3. ${colors.cyan}Detect follow-ups${colors.reset} and handle differently`);
  console.log(`  4. ${colors.cyan}Try different model${colors.reset} if current one ignores instructions`);
  console.log('');
  console.log('='.repeat(100));
  console.log('');
  
  log(colors.green, 'COMPLETE', 'Test finished! Review the prompts above to see what the LLM receives.');
  console.log('');
}
