...
async function testMCP() {
    ...
    console.log('⏳ Waiting 5 seconds for you to start the app if not running...\n');
    ...
    try {
        ...
        await page.screenshot({ path: 'test-screenshots/01-initial-state.png', fullPage: true });
        ...
        const settingsButton = page.locator('button[aria-label*="Settings"], button[title*="Settings"], button:has(svg)').first();
        await settingsButton.waitFor({ state: 'visible', timeout: 5000 });
        await settingsButton.click();
        await page.waitForTimeout(1000);

        await page.screenshot({ path: 'test-screenshots/02-settings-opened.png', fullPage: true });
        ...
        // Look for MCP tab
        const mcpTab = page.locator('button:has-text("MCP"), [role="tab"]:has-text("MCP")');
        if (await mcpTab.isVisible({ timeout: 2000 }).catch(() => false)) {
            await mcpTab.click();
            await page.waitForTimeout(1000);

            await page.screenshot({ path: 'test-screenshots/03-mcp-tab.png', fullPage: true });
        ...
        const mcpToggle = page.locator('button[role="switch"], input[type="checkbox"]').filter({ hasText: 'MCP' }).or(
        ...
        if (await mcpToggle.isVisible({ timeout: 2000 }).catch(() => false)) {
            ...
            await page.screenshot({ path: 'test-screenshots/04-mcp-toggle.png', fullPage: true });
            ...
            if (isChecked === 'false') {
                ...
                await page.screenshot({ path: 'test-screenshots/05-mcp-enabled.png', fullPage: true });
        ...
        await page.screenshot({ path: 'test-screenshots/06-after-close.png', fullPage: true });
        console.log('📸 Screenshot: 06-after-close.png');

        // Look for MCP status indicator in header or somewhere on page
        const mcpIndicator = page.locator('[data-mcp-status], [title*="MCP"], text=/MCP/i').first();
