...
async function testMCP() {
  ...
  try {
    ...
    const mcpTab = page
      .locator('button:has-text("MCP"), [role="tab"]:has-text("MCP")')
      .first();
    ...
    const isEnabled =
      (await mcpToggle.getAttribute("data-state")) === "checked" ||
    ...
    if (logs.length > 0) {
      ...
      console.log(
        "ℹ️  No MCP logs in console (may be normal if already initialized)"
      );
    }

    // Test 6: Check for MCP Button in Header
    console.log("\n📋 Test 6: Checking for MCP Toggle Button in Header...");
    const mcpHeaderButton = page
      .locator('button[title*="MCP"], button[aria-label*="MCP"]')
      .first();
    const headerButtonVisible = await mcpHeaderButton
    ...
    const settingsState = await page.evaluate(() => {
      ...
      if (reactRoot) {
        return {
          found: true,
          note: "React root found but settings not accessible from page context",
        };
      }
      return { found: false };
    });
    ...
    console.log(
      `${isEnabled ? "✅" : "⚠️ "} MCP ${isEnabled ? "was enabled" : "was disabled (now enabled)"}`
    );
    console.log(`${mcpState.success ? "✅" : "❌"} MCP API available`);
    console.log(
      `${mcpState.ready?.ready ? "✅" : "⚠️ "} MCP initialization ${mcpState.ready?.ready ? "successful" : "pending/failed"}`
    );
