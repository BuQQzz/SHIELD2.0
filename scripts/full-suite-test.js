...
async function runFullSuiteTest() {
    ...
    try {
        ...
        const criticalErrors = pageErrors.filter(err =>
            !err.includes('Autofill') && !err.includes('CSP')
        );
        if (criticalErrors.length === 0) {
            logTest('No critical errors on load', 'PASS', 'Clean initialization');
        } else {
            logTest('No critical errors on load', 'FAIL', `${criticalErrors.length} errors found`);
        }
        ...
        try {
            const chatArea = await page.$('[role="main"], main, .chat, [class*="chat"]');
            if (chatArea) {
                logTest('Chat area present', 'PASS', 'Main chat interface found');
            } else {
                logTest('Chat area present', 'WARN', 'Chat area not clearly identified');
            }
        } catch (error) {
        ...
        try {
            const input = await page.$('input[type="text"], textarea, [contenteditable="true"]');
            if (input) {
        ...
        try {
            ...
            if (settingsButton) {
                ...
                logTest('Settings tabs present', tabs.length > 0 ? 'PASS' : 'WARN',
                ...
                for (const tab of tabs) {
                    ...
                    if (text && text.toLowerCase().includes('mcp')) {
                        ...
                        await tab.click();
                        await wait(1000);
                        logTest('MCP tab accessible', 'PASS', 'MCP settings found and opened');
                ...
                if (!mcpFound) {
                    logTest('MCP tab accessible', 'WARN', 'MCP tab not found in settings');
                }
        ...
        try {
            ...
            if (input) {
                ...
                await wait(500);
                logTest('Input accepts text', 'PASS', 'Successfully typed test message');
                await screenshot(page, '06-text-input.png', 'Text entered in chat input');
        ...
        try {
            ...
            logTest('Theme styles applied', 'PASS',
        ...
        logTest('Console warnings check', 'PASS',
        ...
        if (failed === 0 && warnings <= 3) {
            ...
            console.log('The chatbot passed all critical tests and is ready for users!');
        } else if (failed === 0) {
            console.log('⚠️  VERDICT: MOSTLY READY ✅');
            console.log('The chatbot passed all tests but has some warnings to review.');
        } else {
